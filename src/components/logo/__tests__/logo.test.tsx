...
describe('Logo', () => {
  ...
  it('should not have accessibility violations', async done => {
    const { container, unmount } = render(
      <Logo inverted={false} title={logoTitle} desc={logoDesc} />
    );
  ...
  it('it should render with the inverted colors', () => {
    const { container, unmount } = render(
      <>
        <Logo inverted={true} title={logoTitle} desc={logoDesc} />
      </>
    );
    const svg = container.querySelector('svg') as SVGElement;
    const styles = window.getComputedStyle(svg)
    expect(styles.getPropertyValue('color')).toMatch(toRGBString(color.white));
    unmount();
  });
});
