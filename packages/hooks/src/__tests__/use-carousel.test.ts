...
type LazyInitState = (state: CarouselState) => CarouselState

describe("UseCarousel", () => {
  let width: number
  let length: number
  let carouselInitStateOne: LazyInitState
  let carouselInitStateTwo: LazyInitState

  beforeEach(() => {
    width = 375
    length = 6
    carouselInitStateOne = (state: CarouselState) => ({ ...state, length: 4 })
    carouselInitStateTwo = (state: CarouselState) => ({ ...state, length: 9 })
  })

  afterEach(() => {
    width = (undefined as unknown) as number
    length = (undefined as unknown) as number
    carouselInitStateOne = (undefined as unknown) as LazyInitState
    carouselInitStateTwo = (undefined as unknown) as LazyInitState
  })

  describe("useCarousel()", () => {
    describe("reducer initialization", () => {
      it("should return a default coordinate state of 0", () => {
        const { result } = renderHook(() => useCarousel(carouselInitStateOne))
    ...
    describe("jump", () => {
      ...
      it.each([0, 1, 2, 3, 4, 5, 6, 7, 8])(
        ...
        current => {
          const { result } = renderHook(() => useCarousel(carouselInitStateTwo))
