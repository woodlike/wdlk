...
export interface BoxProps {
  ...
  readonly borderColors?: ScaleArea;
}
...
// const createStylesBorderColor = (colorNames: any, qt: ThemeQuery): SxStyleProp => {
//   const colors = Array.isArray(colorNames) ? colorNames.map(name => qt(`${name}`)) : qt(`${colorNames}`);
...
export const Box: React.FC<BoxProps> = (props): JSX.Element => {
  ...
  return <div sx={createStyles(props)}>{props.children}</div>;
};
