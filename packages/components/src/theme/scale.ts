...
export const create = (area: ScaleArea, scale: Scale[]): ScaleBox =>
  createBox(getThemeScale(area, scale));

export const toCSSPixel = (box: ScaleBox): string =>
  box.map(value => `${value}px`).join(' ');
export const toCSSString = (box: ScaleBox): string =>
  box.map(value => `${value}`).join(' ');
